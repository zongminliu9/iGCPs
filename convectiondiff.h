...
private:
	...
	void MTPlus(double x, double y, double z, double &lp, double lp_grad[3]);
	void Tangent1(const int nen, vector<double> &Nx, vector<double> &Npx, vector<double> &Nmx, vector<array<double, dim>> &dNdx, double vp[dim], double vm[dim], double lp, double lp_grad[dim], double detJ, vector<vector<double>> &EMatrixSolve);
	void Residual1(const int nen, const double CA, const double Nplus, const double Nminus, const vector<double> &Nx, const vector<double> &Npx, const vector<double> &Nmx, double lp, double lp_grad[dim], const double detJ, vector<double> &EVectorSolve);

	void GaussInfo(int ng);
	void SUPGcoefficient(double s, double v[3], double dudx[3][3], vector<array<double, 3>>& dNdx, double &tau_supg);
	void BasisFunction(double u, double v, double w, const int nen, const vector<Vertex3D> &cpt, const vector<array<double, 64>> &cmat, vector<double> &Nx, vector<array<double, 3>> &dNdx, double dudx[3][3], double &detJ);
